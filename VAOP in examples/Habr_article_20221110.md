...
Va-engine легко переписывается на любом языке программирования. Самое замечательное, что при этом Va-script остается без изменения. Да, **без изменения остается Алгоритм (Algorithm is model of actions) решения задачи**. Это именно, то о чем я спрашиваю программистов, когда задаю правильный вопрос - а где у Вас Алгоритм? Потому, что в будущем, другому программисту, уже без Вас, придется внести изменения в Алгоритм по просьбе Заказчика. В VAOP c этим нет проблем и Заказчик, посмотрев в Va-script, вспомнит, что он хотел сделать и вместе с программистом внесет изменения в сценарий и, потом программист добавит новые Action. Как все это происходит в реале без VAOP все хорошо знают. Программа как-то работает, но что она делает уверенности нет ни у кого. Внесения изменений со временем превращают программу в Гроб, который легче закопать и забыть и переписать все заново. С VAOP подходом, превратить программу в Гроб невозможно.
